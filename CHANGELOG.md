...
- Auto-build for `compose up`: a service with a `build:` section is now built automatically when its target image (the `<project>-<service>` or `image:` tag that `compose build` would use) is not present locally, instead of failing with "building images is not supported yet" — mirroring `docker compose up`. Existing images are reused by default. New `IContainerProvider.ImageExistsAsync` (shared by `docker`/`wslc`, reusing the local image inspect that `EnsureImageAsync` already performed) and a `base_directory` on `UpRequest` so a build-only service's relative `build.context` resolves against the compose file's directory.
  - `compose up --build` forces a rebuild of every `build:` service before starting (even when the image exists); `compose up --no-build` skips building entirely and fails a `build:` service whose image is missing. The two are mutually exclusive. Carried on `UpRequest` as a new `BuildPolicy` enum (`Auto`/`Always`/`Never`).
- `compose start`/`restart`/`logs` now process the project's containers in `depends_on` order (dependencies first) and `compose stop`/`down` in the reverse (dependents first), matching how `up` orders startup — instead of the provider's listing order. Ordering uses the compose file when one is supplied (`-f`); a project addressed only by `-p` has no dependency graph and keeps listing order.
- `compose start`/`stop`/`restart`/`logs`/`pull`/`build` now reject a `[SERVICES]` name the project does not define with `no such service: <name>` (previously such names were silently ignored / a no-op). For `start`/`stop`/`restart`/`logs` the known set is the compose file's services, or — when only `-p` is given — the services that currently have a container.
