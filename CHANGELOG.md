...
- Client-side compose file resolution: multi-file merge (`-f` repeatable, `COMPOSE_FILE`), `.env` and `${VAR}` interpolation (`--env-file`), `extends`, `profiles` (`--profile`, `COMPOSE_PROFILES`), and `--project-directory`.
