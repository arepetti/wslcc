...
- A WinUI3 application to visualize images/containers, status, metrics, and logs. It talks to the same `wslccd` daemon over gRPC — no direct provider access.
...
- First-class machine-wide service via an MSI installer (today `daemon install` is a per-user logon autostart).
- Move the WSL provider from the `wslc.exe` fallback to the managed `Microsoft.WSL.Containers` SDK as it reaches API parity.
